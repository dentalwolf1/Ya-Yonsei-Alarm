...
# ── PostgreSQL-backed job store (survives Render restarts) ───────────────────
DATABASE_URL = os.environ.get("DATABASE_URL")

if DATABASE_URL:
    from apscheduler.jobstores.sqlalchemy import SQLAlchemyJobStore
    jobstores = {"default": SQLAlchemyJobStore(url=DATABASE_URL)}
    scheduler = BackgroundScheduler(jobstores=jobstores, misfire_grace_time=300)
    print("[scheduler] Using PostgreSQL job store")
else:
    scheduler = BackgroundScheduler(misfire_grace_time=300)
    print("[scheduler] ⚠️ No DATABASE_URL — using in-memory job store (jobs lost on restart)")
...
def webhook():
    ...
    # Parse: phone  date[/time]  event_name (slash or space separator, optional quotes)
    pattern = r'^(\d{10,11})\s+(\d{1,2}/\d{1,2})(?:[/ ](\d{1,2}:\d{2}))?\s+"?([^"]+?)"?\s*$'
