...
# ---------------------------------------------------------------------------
# ★ INPUT FORMAT (must start with "알림 ")
#
#   First time:   알림 01012345678
#   With alarm:   알림 01012345678 06/10/14:30 "팀 미팅"
#   Returning:    알림 06/10/14:30 "팀 미팅"
# ---------------------------------------------------------------------------
...
FORMAT_HELP = (
    ...
    "  알림 01012345678\n\n"
    "바로 알림 설정:\n"
    "  알림 01012345678 06/10/14:30 \"팀 미팅\"\n\n"
    "재사용 (전화번호 등록 후):\n"
    "  알림 06/10/14:30 \"팀 미팅\"\n\n"
    "• 시간은 24시간 형식 (한국 표준시 기준)\n"
    "• 이벤트 10분 전, 5분 전 SMS 발송"
)
...
# 알림 01012345678
PHONE_ONLY_PATTERN = re.compile(r'^알림\s+(01\d{8,9})\s*$')
# 알림 01012345678 06/10/14:30 "title"
WITH_PHONE_PATTERN = re.compile(
    r'^알림\s+(01\d{8,9})\s+(\d{1,2})/(\d{1,2})/(\d{1,2}):(\d{2})\s+"?(.+?)"?\s*$'
)
# 알림 06/10/14:30 "title"
NO_PHONE_PATTERN = re.compile(
    r'^알림\s+(\d{1,2})/(\d{1,2})/(\d{1,2}):(\d{2})\s+"?(.+?)"?\s*$',
    re.UNICODE
)
...
def webhook():
    ...
    if phone_only:
        ...
        return jsonify(send_text_response(
            ...
            f"  알림 월/일/시:분 \"알림 제목\"\n"
            f"예시: 알림 06/10/14:30 \"팀 미팅\""
        ))

    # Case 2: phone + datetime + title
    with_phone = WITH_PHONE_PATTERN.match(utterance)
    if with_phone:
        phone = with_phone.group(1)
        month, day, hour, minute = int(with_phone.group(2)), int(with_phone.group(3)), int(with_phone.group(4)), int(with_phone.group(5))
    ...
    # Case 3: returning user, datetime + title only
    ...
    if no_phone:
        ...
        month, day, hour, minute = int(no_phone.group(1)), int(no_phone.group(2)), int(no_phone.group(3)), int(no_phone.group(4))
